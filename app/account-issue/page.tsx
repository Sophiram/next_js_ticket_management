...
export default function AccountIssuePage() {
    ...
    const handlePrint = async () => {
        ...
        try {
            const res = await fetch("/api/tickets", {
                ...
                body: JSON.stringify({ action: "create", category: "ដោះស្រាយបញ្ហាគណនី" }),
            });
            ...
            if (result.success) {
                ...
                // ដំណើរការបញ្ជាទៅម៉ាស៊ីនព្រីនរបស់ Browser ដោយស្វ័យប្រវត្តិ
                setTimeout(() => {
                    window.print();
                }, 500);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex flex-col items-center justify-center p-6 font-khmer relative overflow-hidden print:bg-white print:p-0">
            ...
            {/* តុបតែងផ្ទៃខាងក្រោយបែបធនាគារទំនើប - លាក់ពេលព្រីន */}
            ...
            <div className="bg-white/95 backdrop-blur-md p-10 rounded-3xl shadow-2xl border border-slate-200 text-center max-w-lg w-full space-y-8 transform transition duration-300 hover:scale-[1.01] print:hidden">
                ...
                {/* សញ្ញា Icon តំណាងឱ្យគណនី */}
                <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center mx-auto shadow-inner">
                ...
                <div className="space-y-3">
                    <h2 className="text-2xl font-black text-slate-800 tracking-wide">សេវាដោះស្រាយបញ្ហាគណនី</h2>
                    <p className="text-slate-500 text-sm leading-relaxed max-w-sm mx-auto">
                        សូមចុចប៊ូតុងខាងក្រោមដើម្បីទទួលបានសំបុត្ររង់ចាំ សម្រាប់សេវាកម្មចាក់សោគណនី ភ្លេចលេខកូដសម្ងាត់ ឬកែប្រែទិន្នន័យផ្ទាល់ខ្លួន។
                    </p>
                </div>
                ...
                <div className="space-y-3 pt-2">
                    <button
                        onClick={handlePrint}
                        disabled={loading}
                        className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-4 px-6 rounded-2xl font-bold shadow-lg shadow-indigo-600/20 transition-all duration-200 active:scale-[0.99] disabled:from-slate-400 disabled:to-slate-400 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-base"
                    >
                        {loading ? (
                            <>
            ...
            {showSuccessModal && printedTicket && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 print:relative print:inset-auto print:bg-white print:p-0 print:backdrop-blur-none">
                    ...
                    {/* ឃ្លាំងសំបុត្រកម្ដៅ (Thermal Ticket Concept Box) - បានថែម id សម្រាប់ទាញព្រីន */}
