...
export default function WithdrawalPage() {
    ...
    const handlePrint = async () => {
        ...
        try {
            const res = await fetch("/api/tickets", {
                ...
                body: JSON.stringify({ action: "create", category: "ដកប្រាក់" }),
            });
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-emerald-950/40 flex flex-col items-center justify-center p-6 font-khmer relative overflow-hidden print:bg-white print:p-0">
            ...
            <div className="absolute bottom-[-20%] right-[-10%] w-[600px] h-[600px] bg-teal-500/10 rounded-full blur-3xl pointer-events-none print:hidden" />
            ...
            <div className="bg-white/95 backdrop-blur-md p-10 rounded-3xl shadow-2xl border border-slate-200 text-center max-w-lg w-full space-y-8 transform transition duration-300 hover:scale-[1.01] print:hidden">
                ...
                {/* សញ្ញា Icon តំណាងឱ្យសេវាកម្មដកប្រាក់ (Cash Withdrawal Icon) */}
                <div className="w-20 h-20 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto shadow-inner">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5h16.5M5.25 7.5h13.5m-12 9a3 3 0 0 1 3-3h3a3 3 0 0 1 3 3m-9 0h9M5.25 21.75h13.5w-13.5Zm10.5-15v3m0 0v3m0-3h3m-3 0h-3" />
                    </svg>
                </div>

                {/* ឈ្មោះសេវាកម្ម និងការពន្យល់ */}
                <div className="space-y-3">
                    <h2 className="text-2xl font-black text-slate-800 tracking-wide">សេវាកម្មដកប្រាក់</h2>
                    <p className="text-slate-500 text-sm leading-relaxed max-w-sm mx-auto">
                        សេវាដកប្រាក់រហ័សតាមបញ្ជរសម្រាប់ចំនួនទឹកប្រាក់ធំៗ។ សូមចុចប៊ូតុងខាងក្រោមដើម្បីទទួលបានសំបុត្ររង់ចាំ។
                    </p>
                </div>
                ...
                <div className="space-y-3 pt-2">
                    <button
                        onClick={handlePrint}
                        disabled={loading}
                        className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white py-4 px-6 rounded-2xl font-bold shadow-lg shadow-emerald-600/20 transition-all duration-200 active:scale-[0.99] disabled:from-slate-400 disabled:to-slate-400 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-base"
                    >
                        {loading ? (
                            <>
                ...
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 print:relative print:inset-auto print:bg-white print:p-0 print:backdrop-blur-none">
                    ...
                    <div id="thermal-ticket-print" className="bg-white text-slate-900 p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center space-y-6 border border-slate-100 relative print:shadow-none print:border-none print:p-0 print:w-[76mm] print:max-w-none">
                        ...
                        <div className="space-y-1 py-2">
                            ...
                            <h1 className="text-6xl font-black text-emerald-500 font-mono tracking-tight print:text-black">
